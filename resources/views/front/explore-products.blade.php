				...
				<div class="row">
					<div class="col-lg-12">
						<nav aria-label="Page navigation example">
							@if ($exploredata->lastPage() > 1)
						  <ul class="pagination">
							<li class="page-item left {{ ($exploredata->currentPage() == 1) ? ' disabled' : '' }}">
							  <a class="page-link" href="{{ $exploredata->url(1) }}" aria-label="Previous">
								<span aria-hidden="true"><i class="ti-arrow-left mr-1"></i>Prev</span>
							  </a>
							</li>
							@for ($i = 1; $i <= $exploredata->lastPage(); $i++)
							<li class="page-item {{ ($exploredata->currentPage() == $i) ? ' active' : '' }}"><a class="page-link" href="{{ $exploredata->url($i) }}">{{ $i }}</a></li>
							@endfor
							<li class="page-item right {{ ($exploredata->currentPage() == $exploredata->lastPage()) ? ' disabled' : '' }}">
							  <a class="page-link" href="{{ $exploredata->url($exploredata->lastPage()) }}" aria-label="Previous">
